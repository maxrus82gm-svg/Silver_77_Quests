...
modded class MissionGameplay
{
    ...
    override void OnMissionStart()
    {
        ...
        // Клиент загружает конфиг (тот же что и сервер из $profile)
        if (GetGame().IsClient())
        {
            Print("[Silver_77_Quests] Client loading config");
            QuestManager.Init();
            QuestTriggerManager.Init();
            m_LastQuestConfigRevision = g_Silver77_QuestConfigRevision;
        }
    }
    ...
    void CreateHintWidget()
    {
        ...
        // Проверка что виджет ещё не создан
    ...
    override void OnUpdate(float timeslice)
    {
        ...
        if (GetGame().IsClient())
        {
            ...
            if (player)
            {
                ...
                if (m_LastQuestConfigRevision != g_Silver77_QuestConfigRevision)
                {
    ...
    void RequestInitialQuestSync(PlayerBase player)
    {
        ...
        QuestManager.RequestConfigSync(player);
        QuestManager.RequestPlayerDataSync(player);
    }
    ...
    void CheckPlayerInTrigger(PlayerBase player)
    {
        ...
        {
            // Всё ещё в зоне - обновляем текст если изменился
            m_CurrentTriggerId = trigger.GetTriggerId();
            SetCurrentQuestIds(trigger.GetAvailableQuests());
    ...
    string GetCurrentQuestTriggerId()
    {
