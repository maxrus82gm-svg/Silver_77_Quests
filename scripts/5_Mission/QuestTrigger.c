...
class QuestTrigger
{
    ...
    void SetupTrigger(vector position, float radius, string questGiverId, array<string> questIds, string hintText = "")
    {
        m_Position = position;
        m_Radius = radius;
...
class QuestTriggerManager
{
    ...
    {
        if (!g_QuestConfig || !g_QuestConfig.triggers)
        {
            Print("[Silver_77_Quests] No triggers found in config!");
            return;
        }
        
        foreach (Silver77_QuestTriggerConfig triggerConfig : g_QuestConfig.triggers)
        {
            if (!triggerConfig)
                continue;
            ...
            trigger.SetupTrigger(pos, triggerConfig.radius, triggerConfig.id, triggerConfig.questIds, triggerConfig.hintText);
