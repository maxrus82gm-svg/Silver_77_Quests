...
class QuestJournalUIMenu extends UIScriptedMenu
{
    ...
    override void Update(float timeslice)
    {
        ...
        if (m_LastConfigRevision != g_Silver77_QuestConfigRevision || m_LastDataRevision != g_Silver77_QuestDataRevision)
        {
            m_LastConfigRevision = g_Silver77_QuestConfigRevision;
            m_LastDataRevision = g_Silver77_QuestDataRevision;
            RefreshQuestList();
    ...
    void RefreshQuestList()
    {
        ...
        foreach (Silver77_Quest quest : g_QuestConfig.quests)
        {
            ...
            if (QuestManager.GetQuestStatus(m_Player, quest.id) != "active")
                continue;
    ...
    void UpdateQuestDetails()
    {
        ...
        bool canComplete = QuestManager.CanCompleteQuest(m_Player, quest.id);
        bool canDeposit = QuestManager.CanDepositQuestItems(m_Player, quest.id);
        ...
        for (int i = 0; i < quest.objectives.Count(); i++)
        {
            ...
            bool ready = QuestManager.PlayerHasItems(m_Player, obj.className, obj.quantity, obj.useItemQuantity);
            if (obj.allowPartialTurnIn)
            {
                ...
                ready = remaining <= 0 || QuestManager.PlayerHasItemAmount(m_Player, obj.className, remaining, obj.useItemQuantity);
