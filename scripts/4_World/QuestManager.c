...
ref Silver77_QuestConfig g_QuestConfig;
ref map<string, ref PlayerQuestData> g_PlayerQuestData;
int g_Silver77_QuestConfigRevision = 0;
int g_Silver77_QuestDataRevision = 0;
...
Silver77_QuestConfig CreateDefaultQuestConfig()
{
    ...
    hunterQuest.offerTriggerIds.Insert("hunter_trigger");
    hunterQuest.completionTriggerIds.Insert("hunter_trigger");
    ...
    fishermanQuest.offerTriggerIds.Insert("fisherman_trigger");
    fishermanQuest.completionTriggerIds.Insert("fisherman_trigger");
    ...
    rasputinQuest.offerTriggerIds.Insert("Rasputin_1_trigger");
    rasputinQuest.completionTriggerIds.Insert("Rasputin_1_trigger");
    ...
    fishermanQuest2.offerTriggerIds.Insert("fisherman_trigger");
    fishermanQuest2.completionTriggerIds.Insert("fisherman_trigger");
...
void NormalizeQuestConfig(Silver77_QuestConfig config)
{
    ...
    foreach (Silver77_QuestTriggerConfig seedTrigger : config.triggers)
    {
        if (!seedTrigger || !seedTrigger.questIds)
            continue;
        
        foreach (string questId : seedTrigger.questIds)
        {
            Silver77_Quest seedQuest = null;
            foreach (Silver77_Quest candidate : config.quests)
            {
                if (candidate && candidate.id == questId)
                {
                    seedQuest = candidate;
                    break;
                }
            ...
            if (seedQuest.offerTriggerIds.Count() == 0 && seedQuest.offerTriggerIds.Find(seedTrigger.id) == -1)
                seedQuest.offerTriggerIds.Insert(seedTrigger.id);
            
            if (seedQuest.completionTriggerIds.Count() == 0 && seedQuest.completionTriggerIds.Find(seedTrigger.id) == -1)
                seedQuest.completionTriggerIds.Insert(seedTrigger.id);
        }
    }
}
...
void LoadQuestConfig()
{
    if (!GetGame().IsServer())
    {
        g_QuestConfig = CreateDefaultQuestConfig();
        NormalizeQuestConfig(g_QuestConfig);
    ...
    if (!FileExist(configPath))
    {
        ...
        g_QuestConfig = CreateDefaultQuestConfig();
        NormalizeQuestConfig(g_QuestConfig);
        JsonFileLoader<Silver77_QuestConfig>.JsonSaveFile(configPath, g_QuestConfig);
        Print("[Silver_77_Quests] Config created at: " + configPath);
    }
    else
    {
        g_QuestConfig = new Silver77_QuestConfig();
        JsonFileLoader<Silver77_QuestConfig>.JsonLoadFile(configPath, g_QuestConfig);
        NormalizeQuestConfig(g_QuestConfig);
        Print("[Silver_77_Quests] Loaded " + g_QuestConfig.quests.Count() + " quests and " + g_QuestConfig.triggers.Count() + " triggers");
    }
}
...
class QuestManager
{
    ...
    {
        LoadQuestConfig();
        g_PlayerQuestData = new map<string, ref PlayerQuestData>;
        g_Silver77_QuestConfigRevision++;
        g_Silver77_QuestDataRevision++;
        Print("[Silver_77_Quests] QuestManager initialized");
    }
    ...
    static PlayerQuestData GetPlayerData(PlayerBase player)
    {
        ...
        if (!g_PlayerQuestData)
            g_PlayerQuestData = new map<string, ref PlayerQuestData>;
        
        if (!g_PlayerQuestData.Contains(steamId))
        {
            ...
            g_PlayerQuestData.Insert(steamId, data);
        }
        
        PlayerQuestData playerData = g_PlayerQuestData.Get(steamId);
    ...
    static void RemoveQuestCompletionItems(PlayerBase player, Silver77_Quest quest)
    {
        if (!player || !quest || !quest.objectives)
            return;
    ...
    static Silver77_QuestTriggerAction GetQuestTriggerAction(Silver77_Quest quest, string actionType, string triggerId)
    {
        if (!quest || !quest.triggerActions || actionType == "" || triggerId == "")
            return null;
        ...
        foreach (Silver77_QuestTriggerAction action : quest.triggerActions)
        {
            ...
            if (action.actionType == actionType && action.triggerId == triggerId)
                return action;
        }
    ...
    static void SpawnQuestRewards(PlayerBase player, Silver77_Quest quest, string rewardTriggerId)
    {
        if (!player || !quest)
            return;
        
        Silver77_QuestTriggerAction rewardAction = GetQuestTriggerAction(quest, "reward", rewardTriggerId);
        if (rewardAction && rewardAction.rewards && rewardAction.rewards.Count() > 0)
        {
            foreach (Silver77_QuestItem actionReward : rewardAction.rewards)
            {
                SpawnQuestItem(player, actionReward);
            }
            return;
        }
    ...
    static bool CanAcceptQuestAtTrigger(PlayerBase player, string questId, string triggerId)
    {
        ...
        return QuestRoleContains(quest.offerTriggerIds, triggerId);
    }
    
    static bool AcceptQuest(PlayerBase player, string questId, string triggerId = "")
    {
        if (!GetGame().IsServer() || !CanAcceptQuestAtTrigger(player, questId, triggerId))
            return false;
    ...
    static bool CanCompleteQuestAtTrigger(PlayerBase player, string questId, string triggerId)
    {
        ...
        if (IsQuestCompletionTriggerDone(player, questId, triggerId))
            return false;
        
        return QuestRoleContains(quest.completionTriggerIds, triggerId);
    }
    
    static bool IsQuestCompletionTriggerDone(PlayerBase player, string questId, string triggerId)
    {
        ...
        PlayerQuestData data = GetPlayerData(player);
        PlayerQuestProgress progress = FindProgress(data, questId);
        return IsCompletionTriggerDone(progress, triggerId);
    }
    
    static bool CanRewardQuestAtTrigger(PlayerBase player, string questId, string triggerId)
    {
        if (!player || questId == "" || triggerId == "")
            return false;
        
        Silver77_Quest quest = GetQuestById(questId);
        if (!quest || !QuestRoleContains(quest.rewardTriggerIds, triggerId))
            return false;
        ...
        return CanDepositQuestItems(player, questId);
    }
    ...
    static bool CompleteQuest(PlayerBase player, string questId, string triggerId = "")
    {
        ...
        if (QuestRoleContains(quest.completionTriggerIds, triggerId) && !IsCompletionTriggerDone(progress, triggerId))
        {
            ...
            if (!QuestHasRoleTriggers(quest.rewardTriggerIds))
                RemoveQuestCompletionItems(player, quest);
            
            MarkCompletionTriggerDone(progress, triggerId);
            changed = true;
            
            Silver77_QuestTriggerAction completionAction = GetQuestTriggerAction(quest, "completion", triggerId);
            if (completionAction && completionAction.rewards)
            {
                foreach (Silver77_QuestItem completionReward : completionAction.rewards)
                {
                    SpawnQuestItem(player, completionReward);
                }
            }
            ...
            if (!QuestHasRoleTriggers(quest.rewardTriggerIds) && AreAllCompletionTriggersDone(quest, progress))
            {
                SpawnQuestRewards(player, quest, triggerId);
            ...
            SavePlayerData(data);
            g_Silver77_QuestDataRevision++;
            Print("[Silver_77_Quests] Player completed quest stage: " + questId + " at trigger: " + triggerId);
            return true;
        }
        ...
        if (QuestRoleContains(quest.rewardTriggerIds, triggerId))
        {
            ...
            bool rewardDepositedSomething = DepositPartialQuestItems(player, quest, progress);
            ...
            SavePlayerData(data);
            g_Silver77_QuestDataRevision++;
            Print("[Silver_77_Quests] Player received reward and completed quest: " + questId + " at trigger: " + triggerId);
            return true;
        }
    ...
    static bool RequestAcceptQuest(PlayerBase player, string questId, string triggerId = "")
    {
        ...
        Print("[Silver_77_Quests] Sending accept quest RPC: " + questId + " at trigger: " + triggerId);
    ...
    static bool RequestCompleteQuest(PlayerBase player, string questId, string triggerId = "")
    {
        ...
        Print("[Silver_77_Quests] Sending complete quest RPC: " + questId + " at trigger: " + triggerId);
    ...
    {
        if (!GetGame().IsServer() || !player || !player.GetIdentity() || !g_QuestConfig)
            return;
        
        GetGame().RPCSingleParam(player, SILVER77_QUEST_RPC_CONFIG_DATA, new Param1<Silver77_QuestConfig>(g_QuestConfig), true, player.GetIdentity());
    ...
    static void ApplySyncedConfig(Silver77_QuestConfig config)
    {
        ...
        g_QuestConfig = config;
        g_Silver77_QuestConfigRevision++;
        
        int questCount = 0;
        int triggerCount = 0;
        if (g_QuestConfig.quests)
            questCount = g_QuestConfig.quests.Count();
        if (g_QuestConfig.triggers)
            triggerCount = g_QuestConfig.triggers.Count();
    ...
    static void ApplySyncedPlayerData(PlayerQuestData data)
    {
        ...
        EnsurePlayerProgress(data);
        g_PlayerQuestData.Set(data.steamId, data);
        g_Silver77_QuestDataRevision++;
