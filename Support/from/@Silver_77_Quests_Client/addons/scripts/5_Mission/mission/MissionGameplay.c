...
modded class MissionGameplay
{
    ...
    private string m_CurrentTriggerId = "";
    ...
    override void OnMissionStart()
    {
        ...
        // Client loads local defaults, then syncs with the server.
        if (GetGame().IsClient())
        {
    ...
    void CreateHintWidget()
    {
        ...
        // Ensure the widget has not been created yet.
    ...
    override void OnUpdate(float timeslice)
    {
        ...
        if (GetGame().IsClient())
        {
            ...
            if (player)
            {
                ...
                // Keep the trigger context stable while quest UI is open.
                if (!IsQuestInterfaceOpen())
                {
                    // Check triggers every 0.5 seconds.
    ...
    bool IsQuestInterfaceOpen()
    {
        UIManager uiManager = GetGame().GetUIManager();
        if (!uiManager)
            return false;

        return uiManager.IsMenuOpen(MENU_QUEST_UI) || uiManager.IsMenuOpen(MENU_QUEST_JOURNAL_UI);
    }
    ...
    {
        if (key == KeyCode.KC_ESCAPE && GetGame().GetUIManager().IsMenuOpen(MENU_QUEST_UI))
        {
            if (m_QuestMenu)
            {
                Print("[Silver_77_Quests] ESC pressed, closing quest menu");
                CloseQuestMenu();
                return;
            }
            
            Print("[Silver_77_Quests] ESC pressed, quest menu is open but local reference is null");
        }
        ...
        {
            PlayerBase player = PlayerBase.Cast(GetGame().GetPlayer());
            QuestTrigger focusedTrigger = QuestTriggerManager.GetFocusedTriggerForPlayer(player);
            if (!focusedTrigger)
            {
                Print("[Silver_77_Quests] F pressed, but player is not looking at quest trigger");
                return;
            }
            ...
            SetCurrentQuestIds(focusedTrigger.GetAvailableQuests());
            m_CurrentTriggerId = focusedTrigger.GetTriggerId();
            m_CurrentHintText = focusedTrigger.GetHintText();
            Print("[Silver_77_Quests] F pressed while looking at quest trigger");
    ...
    {
        QuestTrigger trigger = QuestTriggerManager.GetFocusedTriggerForPlayer(player);
        
        if (trigger && !m_PlayerInQuestZone)
        {
            // Entered the zone.
            m_PlayerInQuestZone = true;
            m_CurrentTriggerId = trigger.GetTriggerId();
        ...
        {
            // Still in the zone: update text if it changed.
            m_CurrentTriggerId = trigger.GetTriggerId();
            SetCurrentQuestIds(trigger.GetAvailableQuests());
        ...
        {
            // Left the zone: hide the hint immediately.
